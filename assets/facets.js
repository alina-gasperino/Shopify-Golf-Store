class FacetFiltersForm extends HTMLElement {
  ...
  static renderFilters(html, event) {
    ...
    if($(parsedHTML).find('price-range').find('.range-group__slider-label').length){
      const facetPrice = parsedHTML.querySelector('price-range').querySelector('.range-group__slider-label');
      $(document).find('.range-group__slider-label').html(facetPrice.innerHTML);
    }
  ...
  static renderActiveFacets(html) {
    ...
    activeFacetElementSelectors.forEach((selector) => {
      ...
      var checkActiveLength = $(activeFacetsElement).find('facet-remove').length;
      if(checkActiveLength > 1){
        $(document).find('.active-facets').removeClass('hide');
      }else{
        $(document).find('.active-facets').addClass('hide');
      }
    })
...
$(document).ready(function(){
  $(document).on('change','#SortByMobile',function(){
    var thisValue = $(this).val();
    $(document).find('.mobile-facets__checkbox[value="'+thisValue+'"]').trigger('change').trigger('click');
    $(document).find('#SortBy').find('option[value="'+thisValue+'"]').prop('selected',true);
  });
  if($(document).find('.facets__wrapper-horzontal').length && !$(document).find('.position__right').length){
    var getElementWidth = $(window).width();
    $(document).find('.facets__wrapper-horzontal').find('details').each(function(){
      var elementPosition = $(this).offset().left;
      var initialWidth = parseInt(getElementWidth) - parseInt(elementPosition);
      if(initialWidth < 350){
        $(this).addClass('position__right');
      }else{
